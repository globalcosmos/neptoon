...
if st.button("Process data!"):
    ...
    with st.spinner("Processing..."):
        ...
        st.session_state.config_obj = erfy.run_full_process(
            st.session_state.config_obj
        )
        st.session_state["data"] = (
            st.session_state.config_obj.data_hub.crns_data_frame
        )

    tab1, tab2 = st.tabs(
        [":material/Table: Processed data", ":material/show_chart: Plots"]
    )
