...
def read_files(
    sampling_csv="c:/Users/schroen/Projects/Neptoon/neptoon/tests/calibration/mock_data/Sheepdrove2-calibration.csv",
    crns_csv="c:/Users/schroen/Projects/Neptoon/neptoon/tests/calibration/mock_data/Sheepdrove2-CRNS.csv",
    *args,
    **kwargs,
):
    import numpy as np
    ...
    cwd = Path.cwd()
    if isinstance(sampling_csv, str):
        sampling_df_path = (
            cwd / "tests" / "calibration" / "mock_data" / sampling_csv
        )
    else:
        sampling_df_path = sampling_csv
    ...
    for calibration_day in calibration_days:
        ...
        for pid in profile_ids:
            df = calibration_day_data[
                calibration_day_data["Profile_ID"] == pid
            ]
    ...
    if isinstance(crns_csv, str):
        crns_df_path = cwd / "tests" / "calibration" / "mock_data" / crns_csv
    else:
        crns_df_path = crns_csv
    ...
    indices = CRNS_data.index.get_indexer(
        list(calibration_days), method="nearest"
    )
    ...
    st.session_state["data_calib"] = calibration_data
    st.session_state["data_crns"] = CRNS_data
    # st.session_state["data_calib_processed"] = df
    ...
    for index, row in df.iterrows():
        ...
        print(
            "Calibration day %s" % calibration_day.strftime("%Y-%m-%d %H:%M")
        )
        ...
        df.loc[
            df["calibration_day"] == calibration_day, "footprint_radius"
        ] = footprint_m
    ...
    logger.info("✅ Finished Weighting")

    df = df.drop("profiles", axis=1)
    st.session_state["data_calib_processed"] = df
    ...
    st.session_state["text_N0_calibrated"] = (
        f"Optimum: N0={N0_optimal:.0f} cph, RMSE={RMSE:.3f} m³/m³"
    )
